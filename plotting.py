...
def plotErrors(error1, error2, error3, f, a, exact, dataPlot, x0, X):
    ...
    a.tick_params(axis='both', which='major', labelsize=3)
    a.set_xlabel('X', fontsize = 5)
    a.set_ylabel('Method error', fontsize = 5)
...
def plot_total_errors(error1, error2, error3, f, a, dataPlot, n0, n1):
    ...
    err_line1, = a.plot(range(n0, n1+1), error1)
    err_line2, = a.plot(range(n0, n1+1), error2)
    err_line3, = a.plot(range(n0, n1+1), error3)
    a.legend([err_line1, err_line2, err_line3],
             ['Euler method', 'Improved Euler method', 'Runge-Kutta method'], loc=4, prop={'size': 4})

    axes = f.gca()
    axes.set_xlim([n0, n1])
    a.tick_params(axis='both', which='major', labelsize=3)
    a.set_xlabel('Error', fontsize = 5)
    a.set_ylabel('N', fontsize = 5)
